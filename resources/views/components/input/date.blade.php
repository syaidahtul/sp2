    ...
    <div x-data="{ value: @entangle($attributes->wire('model')), picker: undefined }" 
        x-init="new Pikaday({
            ...
            onSelect: function(date) {
                $refs.input.value = moment(date.toString()).format('MM-YYYY');
            },
            onOpen() { 
        ...
        <input {{ $attributes->whereDoesntStartWith('wire:model') }} x-ref="input" x-bind:value="value"
            class="flex-1 block w-full px-4 py-2 transition duration-150 ease-in-out border rounded-none rounded-r-md form-input sm:text-sm sm:leading-5 focus:outline-none focus:bg-white focus:border-sky-300" />
    </div>
@else
     <div x-data="{ value: @entangle($attributes->wire('model')), picker: undefined }" 
        x-init="new Pikaday({
            ...
            format: 'MM-YYYY',
            onSelect: function(date) {
                $refs.input.value = moment(date.toString()).format('DD-MM-YYYY');
            } ,
